...
def set_endstops(argv):
    '''A short program that sets the endstops
    '''
    ...
    print('Setting end stops at:')

    for device in devices:
        #device.restore_settings()
        current_position = device.get_current_position(blocking = True)
        print str(current_position)
        device.set_maximum_range(current_position)
