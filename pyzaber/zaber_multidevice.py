...
class zaber_multidevice(zaber_device):
    ...
    run_mode: Defines whether the device should run through every command that gets placed
    ...
    def get_current_position(self):
        ''' zaber_multidevice.get_position()

        Returns the current position of all axis as a dictionary.
        '''
        position = {}

        for each_device in self.devices:
            position[each_device] = self.devices[each_device].get_current_position(blocking = True) \
                    / self.devices[each_device].microsteps_per_unit
...
def zaber_multidevice_example(io):
    ...
    multidevice_system.move_relative({'1':10000, '2':20000, '3':0})
    multidevice_system.move_relative({'1':100000, '2':200000, '3':0})
    
    #multidevice_system.devices['1'].set_soft_high_stop(150000)
